...
7. **Análisis de datos categóricos (OE4):** χ² de independencia sobre `nivel_laip` (Pleno/Limitado/No_cumple); prueba exacta de Fisher (2×2) o χ² Monte Carlo (R×C) cuando hay frecuencias esperadas <5; V de Cramér para la magnitud; y regresión logística binaria (`statsmodels`) de `cumple_mayoria_LAIP` y `tiene_vulnerabilidad` con coeficientes, odds ratios, IC95, AIC, BIC y pseudo-R² de McFadden. Para respetar la regla de eventos por predictor (muestra pequeña), cada logística **selecciona hasta 2–3 predictores por significancia bivariada** y los añade uno a uno con una **selección hacia adelante guardada por convergencia y estabilidad** (`regresion_logistica_seleccionada`): un predictor solo entra si el modelo resultante converge y sus coeficientes/errores quedan acotados (evita meter todos a la vez y provocar **separación cuasi-perfecta**). Los predictores descartados (p. ej. `departamento`, 6 niveles) se registran con su motivo. Si aún así ningún subconjunto converge, se marca `convergio=False`, los OR/IC no finitos se reportan como `NaN` (no `inf`) y una nota remite a χ²/Fisher/Cramér, donde recae la inferencia válida.
